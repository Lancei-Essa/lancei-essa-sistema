...
const startServer = async () => {
  ...
  // Registrar rotas de sincronização do YouTube
  const youtubeSyncRoutes = require('./routes/youtube/syncRoutes');
  app.use('/api/youtube/sync', youtubeSyncRoutes);
  console.log('✅ Rotas de sincronização do YouTube ativadas: /api/youtube/sync');
  ...
  app.listen(PORT, () => {
    ...
    // Inicializar sistema de sincronização do YouTube
    try {
      const youtubeSyncInitializer = require('./services/youtube/initializer');
      youtubeSyncInitializer.initialize();
      console.log('✅ Sistema de sincronização do YouTube ativado');
    } catch (error) {
      console.warn('⚠️ Sistema de sincronização do YouTube não pode ser iniciado:', error.message);
    }
  });
};
...
process.on('SIGTERM', () => {
  console.log('[Server] Recebido sinal SIGTERM, desligando serviços...');
  
  // Desligar o sistema de sincronização do YouTube
  try {
    const youtubeSyncInitializer = require('./services/youtube/initializer');
    youtubeSyncInitializer.shutdown();
    console.log('[Server] Sistema de sincronização do YouTube desligado');
  } catch (shutdownError) {
    console.error('[Server] Erro ao desligar sistema de sincronização:', shutdownError);
  }
  
  // Fechar conexão com o MongoDB
  if (mongoose.connection.readyState) {
    mongoose.connection.close().then(() => {
      console.log('[Server] Conexão MongoDB fechada com sucesso');
      process.exit(0);
    }).catch(err => {
      console.error('[Server] Erro ao fechar conexão MongoDB:', err);
      process.exit(1);
    });
  } else {
    process.exit(0);
...
process.on('SIGINT', () => {
  console.log('[Server] Recebido sinal SIGINT, desligando serviços...');
